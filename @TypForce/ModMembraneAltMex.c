...
//==========================================================================================================================
double getHelfrich (int m_ver, double ver[m_ver][3], double ver_alt[m_ver][3], int* n_node, double* kH, double* kG, double* A, int* idx, int m_idx, int n_jT, int j_T[m_ver][n_jT], int j_T_alt[m_ver][n_jT]) {
double K[m_ver][3];
double H_alt=0.; double H = 0.;
//-------------------------------------------------------------------------
double kH_alt[m_ver]; double A_alt[m_ver]; double kG_alt[m_ver];
for (int m = 0; m < m_ver; m++) {
    kH_alt[m] = 0.; A_alt[m] = 0.; kG_alt[m]=0.;
}
//-------------------------------------------------------------------------
...
//------------------------------------------------------------------------- 
    ...
    kG_alt[i]=(2.0*3.1415926-kG_alt[i])/A_alt[i];
//-------------------------------------------------------------------------
} // i
for (int i_ver=0; i_ver<m_idx; i_ver++){
    int i = idx[i_ver];
    H_alt += 0.5*(2*kH_alt[i]*2*kH_alt[i])*A_alt[i];
    H_alt += kG_alt[i]*A_alt[i];
}
    return H_alt-H;
}
//==========================================================================================================================
...
double K[m_ver][3]; double kH[m_ver]; double kG[m_ver];
for (int m = 0; m < m_ver; m++) {
    kH[m] = 0.; A[m] = 0.; kG[m] = 0.;
    for (int k = 0; k < 3; k++) {
        K[m][k] = 0;
...
for (int iJ=0; iJ<m_J; iJ++){
    ...
    for (int k=0; k<3; k++){
     ver_alt[i][k] += dr_H; 
     dH = getHelfrich (m_ver, ver, ver_alt, n_node, kH, kG, A, idx_new, n_idx_keep, n_jT, j_T, j_T_alt);
