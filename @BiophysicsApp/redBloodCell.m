function [] = redBloodCell(dirRoot,dirMod,varargin)
%--------------------------------------------------------------------------
        % tetherPull performs the red blood cell (RBC) formation from a sphere case study
        % input: 
        ...
        %   exampleFilopodia, exampleEndocytosis
...
M = model(int_info,dirMod,u,[-50 50; -50 50; -50 50],m);
    ...
    M.mod{M.i_mod.ModMembrane}.pm.V0=1223*0.6; %60% of spherical volume
...
%--------------------------------------------------------------------------
%% dynamics
fprintf('computing red blood cell morphology...\n');
parfor iPar=1:nPar
% for iPar=1:1    
    M=Mpar{iPar};
