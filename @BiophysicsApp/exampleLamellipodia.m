function [] = exampleLamellipodia(dirRoot,dirMod,varargin)
%--------------------------------------------------------------------------
        % exampleLamellipodia performs the example of forming Lamellipodia
        % input: 
        % dirRoot - root directory for data storage
        % dirMod - directory of @ModMembrane and other required objects
        % optional:
        % see variable arguments
        %   See also tetherPull, membraneFussion, exampleFilopodia, exampleEndocytosis, redBloodCell
...
%--------------------------------------------------------------------------
%dynamics
fprintf('computing Lamellipodia morphology...\n');
parfor iPar=1:nPar
% for iPar=1:1    
    M=Mpar{iPar};
    ...
    extendedLamellipodia=false;
    while (i_loop_done<nStep)
        [M,CutOff] = TimeEval(dyn,M,Fname,'tCutoff',Tcutoff,'sMax',10);
        i_loop_done=i_loop_done+CutOff.nS;
        %extend Lamellipodia after half simulation
        if (i_loop_done>nStep/2) && (extendedLamellipodia==false)
            M.mod{M.i_mod.ModSubstrate}.var.coord(1:30,:)=M.mod{M.i_mod.ModSubstrate}.var.coord(1:30,:)*1.2;
            extendedLamellipodia=true;
        end
