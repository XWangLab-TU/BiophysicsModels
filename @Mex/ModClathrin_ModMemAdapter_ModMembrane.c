...
//-------------------------------------------------------------------------  
                ...
                for (int m=0; m<n_jT; m++) {
                    if (j_T[i_m][m]>-0.5) {
                        for (int mm=0; mm<3; mm++) {
                        coord_tem[mm]=coord_m[j_T[i_m][m]][mm];
                        }
                        V_alt=potential(coord_tem[0], coord_tem[1], coord_tem[2], foot[ift][0], foot[ift][1], foot[ift][2], pm[0]);
                        dV_adp[m]=V_alt-V;   
                        if (dV_adp[m]<0){
                ...
                if (change_adp==true){
                  i_min=getMinID (dV_adp, n_jT);
                  new_adp[i_m]=j_T[i_m][i_min];
                }
//-------------------------------------------------------------------------                
            }
        }
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    double *outMatrix4;      plhs[3] = mxCreateDoubleMatrix((mwSize)m_coord_m,1,mxREAL);          outMatrix4 = mxGetPr(plhs[3]);
