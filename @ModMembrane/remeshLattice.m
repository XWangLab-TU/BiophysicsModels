...
while (changed == true)
%----------------------------------------------------------------------------------------
        meshIDedg=lc.component{iM}.meshID(M.mod{iM}.var.edge_all);
        ...
        idTooShort=dist<M.mod{iM}.pm.Vedg.rb_1;
        idTooShort=id_all(idTooShort);
        idTooLong=dist>M.mod{iM}.pm.Vedg.rb_2;
...
%----------------------------------------------------------------------------------------        
        if nRemesh > 0
            ...
            if SplitOrMerge(iRemesh) == 1 %split
                ...
                [~,j_edg]=min(dist);
            %--------------------------------------------------------------
            elseif SplitOrMerge(iRemesh) == 2 %merge
                modSave=M;
                if merge.can(i_edg)
                end
                [~,j_edg]=max(dist);
...
%--------------------------------------------------------------------------
                ...
                [M,~] = remeshLocRelax(m,M,M.mod{i_mod}.var.edge_all(i_edg,:),'local',1,'D',100,'ring_ord', 1);
                if print_or_not==true
                fprintf('failed: unmergable\n'); disp([min(M.mod{i_mod}.var.CanSplitMerge), nTry]);
                end
            end
%--------------------------------------------------------------------------
        end
