function [id_ring_ver1,id_ring_edg1,id_ring_ver2,id_ring_edg2] = remeshRing(m,edge_ctr, varargin)
%--------------------------------------------------------------------------
        % remeshRing performs the identification of 1-ring or 2-ring neighbors on 
        % @ModMembrane aroung a given edge
        % input: 
        % m - @ModMembrane input
        % edge_ctr - index of edge to find the neighbors
        % output:
        % id_ring_ver1 - 1-ring neighbor index of vertices
        % id_ring_edg1 - 1-ring neighbor index of edges
        % id_ring_ver2 - 2-ring neighbor index of vertices
        % id_ring_edg2 - 2-ring neighbor index of edges
        % optional:
...
ip.addParameter('mex_avail', false, @islogical); %mex currently not supported
ip.parse(m, edge_ctr, varargin{:});
%--------------------------------------------------------------------------------------------------------
plot_or_not = ip.Results.plot_or_not;
%%
if ip.Results.mex_avail==true
     error('mex currently not supported');
%     [id_ring_ver1,id_ring_edg1,n_ring_ver1,n_ring_edg1]=m.ModMembrane8RingMex(m.var.edge_all,edge_ctr);
...
%         [id_ring_ver_add,id_ring_edg_add,n_ring_ver1,n_ring_edg1]=m.ModMembrane8RingMex(m.var.edge_all,edge_ctr_add);
