...
%--------------------------------------------------------------------------
        % SetVar performs the variable setup for @ModMembrane
        % input: 
...
ip.addParameter('n_adp', [], @isnumeric);
...
var = struct(...
    ...
    'id_adp', [],...
    'n_adp', [],...
    'CanSplitMerge', [],... %index of splitable and mergeable edges
    'idMesh',[]...
     );
 if update == false
...
if update == false
    ...
    var.CanSplitMerge=ones(var.n_edg,2);
end
%================================================================================
...
if update==false
   var.n_adp=n_adp;
   var.id_adp=randsample(var.id_on_coord,n_adp);
end
obj.var=var;
[obj] = getUface(obj);
%--------------------------------------------------------------------------
